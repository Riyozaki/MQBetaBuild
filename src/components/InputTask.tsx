...
const InputTask: React.FC<Props> = ({ task, onAnswer, onHint, isHinted = false, hintsRemaining = 0 }) => {
    ...
    const checkAnswer = () => {
        ...
        // ===== ФИЛЬТР =====
        if (task.type !== 'number_input' && !contentFilter.isClean(userVal)) {
            toast.error('Ответ содержит недопустимые слова 🚫');
            return;
        }
        // ===== КОНЕЦ =====
        ...
        if (task.type === 'number_input') {
            ...
            if (!isCorrect && task.acceptableAnswers) {
                isCorrect = task.acceptableAnswers.some((alt: string) => compare(userVal, alt.trim()));
            }
        }
