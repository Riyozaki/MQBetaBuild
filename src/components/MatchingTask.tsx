...
const MatchingTask: React.FC<Props> = ({ task, onAnswer, onHint, isHinted = false, hintsRemaining = 0 }) => {
    ...
    const [leftItems] = useState(() => shuffle(pairs.map((p: any) => p.left)));
    const [rightItems] = useState(() => shuffle(pairs.map((p: any) => p.right)));
    ...
    const handleRightClick = (item: string) => {
        ...
        const pair = pairs.find((p: any) => p.left === selectedLeft);
    ...
    const handleHint = () => {
        ...
        const unmatchedPairs = pairs.filter((p: any) => !matched.has(p.left));
